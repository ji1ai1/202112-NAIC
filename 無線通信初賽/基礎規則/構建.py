...
class Encoder(torch.nn.Module):
	def __init__(self, feedback_bits, data_size=10000):
		...
		self.indices = torch.nn.Parameter(torch.zeros((self.N * data_size, 1), dtype=torch.int), requires_grad=False)
		self.data = torch.nn.Parameter(torch.zeros((self.N * data_size, 1), dtype=torch.half), requires_grad=False)
